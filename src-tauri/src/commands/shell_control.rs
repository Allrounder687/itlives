...
mod win32 {
    ...
    use windows::Win32::UI::WindowsAndMessaging::{
        EnumWindows, FindWindowExW, FindWindowW, ShowWindow, SW_HIDE, SW_SHOW,
    };
        ...
        unsafe {
            ...
            if let Ok(p) = progman {
                if !p.is_invalid() {
                    let shelldll: Vec<u16> = "SHELLDLL_DefView\0".encode_utf16().collect();
                    let shell =
                        FindWindowExW(p, HWND(0 as _), PCWSTR(shelldll.as_ptr()), PCWSTR::null());
    ...
    pub fn set_taskbar_blur(enable: bool, use_acrylic: bool) -> Result<(), String> {
        unsafe {
            ...
            let user32 = windows::Win32::System::LibraryLoader::LoadLibraryW(PCWSTR(
                "user32.dll\0".encode_utf16().collect::<Vec<u16>>().as_ptr(),
            ))
            ...
            if let Some(func) = set_window_composition_attribute {
                let func: extern "system" fn(HWND, *mut WINDOWCOMPOSITIONATTRIBDATA) -> BOOL =
