...
pub fn export_itl_package(config_json: String, dest_path: String) -> Result<(), String> {
    #[cfg(windows)]
    {
        ...
        let video_src = config["videoSrc"].as_str().ok_or("No videoSrc found in config")?.to_string();
        ...
        // Write config
        fs::write(temp_dir.join("config.json"), serde_json::to_string(&config).unwrap()).map_err(|e| e.to_string())?;
        ...
        cmd.args(&[
            "-NoProfile", 
            "-Command", 
            &format!("Compress-Archive -Path '{}' -DestinationPath '{}' -Force", source_pattern, temp_zip_path.display())
        ]);
        cmd.creation_flags(0x08000000); // CREATE_NO_WINDOW
        
        let output = cmd.output().map_err(|e| format!("PowerShell failed to execute: {}", e))?;
        
        if output.status.success() {
            // Rename the temp zip to the actual destination path (.itl)
            fs::rename(&temp_zip_path, &dest_path).or_else(|_| {
            ...
            Err(format!("Failed to create .itl package. PowerShell error: {}\n{}", stderr, stdout))
        }
    }
    #[cfg(not(windows))]
    {
...
#[tauri::command]
pub fn import_itl_package(state: tauri::State<'_, crate::wallpaper::state::AppStateStore>, src_path: String) -> Result<String, String> {
    #[cfg(windows)]
    {
        use std::os::windows::process::CommandExt;
        ...
        let temp_zip_path = temp_dir.with_extension("zip");
        fs::copy(&src_path, &temp_zip_path).map_err(|e| format!("Failed to copy source file: {}", e))?;
        
        let mut cmd = std::process::Command::new("powershell");
        cmd.args(&[
            "-NoProfile", 
            "-Command", 
            &format!("Expand-Archive -Path '{}' -DestinationPath '{}' -Force", temp_zip_path.display(), temp_dir.display())
        ]);
        cmd.creation_flags(0x08000000); // CREATE_NO_WINDOW
        
        let status = cmd.status().map_err(|e| format!("PowerShell failed: {}", e))?;
        ...
        let imported_dir = PathBuf::from(&local_app_data).join("itLives").join("wallpapers").join("imported");
        ...
        // Save as profile
        let original_filename = PathBuf::from(&src_path).file_stem().unwrap().to_string_lossy().to_string();
        let profile_name = format!("{} (Imported)", original_filename);
        
        super::profiles::save_profile(profile_name.clone(), serde_json::to_string(&config).unwrap())?;
        
        // Add to Library Imports
        let thumbnail_url = config.get("thumbnailUrl").and_then(|v| v.as_str()).unwrap_or("").to_string();
        let safe_name = profile_name.replace(|c: char| !c.is_alphanumeric() && c != ' ' && c != '-', "_");
